...
仅将模型权重加载到内存中就需要约 2.4 GB RAM。在推理期间，你还需要内存用于：
...
FFN 是每个块中最大的组件（约 9.4M，总共 15.7M）。注意力投影每个块约占 6.3M（q_proj、k_proj、v_proj、o_proj 合计），FFN 每个块约占 9.4M。嵌入（由于权重绑定也用作 lm_head）约占所有参数的 26%，这是大词汇表（151,936 个 token）的结果。
